...
void OnRadioCadDone( bool channelActivityDetected )
{
	...
	{
		LoRaCsma.Cad_Detect = true;
		LoRaCsma.Cad_Done = true;
		
		DEBUG_APP(2,"Cad_State3333 = Cad_Detect \r\n");
	}else
	{
		LoRaCsma.Cad_Done = true;
		DEBUG_APP(3,"Cad_State3333 = CadDone \r\n");
	}
}
...
static void OnRadioTxDone( void )
{
   ...
   if( LoRaMacDeviceClass != CLASS_C )
    ...
    {
    	DEBUG(3, "IsRxWindowsEnabled is true RxWindow1Delay : %d\r\n",RxWindow1Delay);
        ...
        if( LoRaMacDeviceClass != CLASS_C )
        {
            ...
            TimerStart( &RxWindowTimer2 );
				...
				DEBUG(3, "LoRaMacDeviceClass : %d, NodeAckRequested : %d\r\n",LoRaMacDeviceClass,NodeAckRequested);
    ...
    switch( macHdr.Bits.MType )
    {
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_DOWN:
            {					
							...
							{
								if(LoRaMacDevAddr != 0xFFFFFFFF && LoRaCsma.Disturb) ///干扰信号：触发重发机制，而非再次上传
								{        
									 TimerStop( &CsmaTimer );									
                ...
                ///  else if() send Done 则不是侦听模式，处于接收模式接收到非网关应答数据，需要开启重发保护机制
								else if(LoRaMacDevAddr != 0xFFFFFFFF && !LoRaCsma.Disturb)
								{
									Sx127xHandle.PackageCounter ++;
									
									if(Sx127xHandle.PackageCounter <= 2)
									Sx127xHandle.SendAgain = true;
...
//                               appSKey = curMulticastParams->AppSKey;
								...
								if( multicast == 0 )
								{
									...
									DEBUG(2,"LoRaCsma.Disturb RxDoneAbort return\r\n");
									return;
								 }
							}
							else
...
static void OnRadioRxError( void )
{
    ...
    {
//        CsmaTimerEvent = false;
//        int32_t Csmatime = randr(-5, 5)* TIMEONAIR;
//        TimerStop( &CsmaTimer );
//        TimerSetValue( &CsmaTimer, 5 * TIMEONAIR + Csmatime); //+ randr(-1.5*TimeOnAir, 0)
//        TimerStart( &CsmaTimer );
...
static void OnRadioRxTimeout( void )
{
	...
	{
		///只针对RX1标志位操作，其它过滤不使用
		if(Sx127xHandle.OnRxWindow1)
		{
			DEBUG_APP(2,"OnRadioRxTimeout ACK Fail");
			Sx127xHandle.OnRxWindow1 = false;
...
static void OnMacStateCheckTimerEvent( void ) ///LoRaMacPrimitives->MacMcpsIndication( &McpsIndication );
{
	...
	if( LoRaMacFlags.Bits.MacDone == 1 )
	{
		...
		if( ( LoRaMacFlags.Bits.MlmeReq == 1 ) || ( ( LoRaMacFlags.Bits.McpsReq == 1 ) ) )
		{
					...
					( MlmeConfirm.Status == LORAMAC_EVENT_INFO_STATUS_TX_TIMEOUT ) )
			{
				...
				txTimeout = true;
				DEBUG_APP(2, "LoRaMacState &= ~MAC_TX_RUNNING--2");
			}
		}
		...
		if( ( NodeAckRequested == false ) && ( txTimeout == false ) )
		{
			...
			if( ( LoRaMacFlags.Bits.MlmeReq == 1 ) || ( ( LoRaMacFlags.Bits.McpsReq == 1 ) ) )
			{
				if( ( ChannelsNbRepCounter >= ChannelsNbRep ) || ( LoRaMacFlags.Bits.McpsInd == 1 ) )
				{
					...
					DEBUG_APP(2,"LoRaMacState &= ~MAC_TX_RUNNING--3");
					LoRaMacState &= ~MAC_TX_RUNNING;
				}
				else
				{
		...
		if( LoRaMacFlags.Bits.McpsInd == 1 )
		{
			if( ( McpsConfirm.AckReceived == true ) || ( AckTimeoutRetriesCounter > AckTimeoutRetries ) )
			{
				...
				DEBUG_APP(2,"LoRaMacState &= ~MAC_TX_RUNNING--4");
				LoRaMacState &= ~MAC_TX_RUNNING;
			}
		}
		...
		if( ( AckTimeoutRetry == true ) && ( ( LoRaMacState & MAC_TX_DELAYED ) == 0 ) )
		{
			...
			if( ( AckTimeoutRetriesCounter < AckTimeoutRetries ) && ( AckTimeoutRetriesCounter <= MAX_ACK_RETRIES ) )
			{
				...
				if( (( AckTimeoutRetriesCounter % 2 ) == 1 ) )
				{
					...
					DEBUG_APP(3," MAX( ChannelsDatarate - 1, LORAMAC_MIN_DATARATE ) = %d", MAX( ChannelsDatarate - 1, LORAMAC_MIN_DATARATE ));
				}
					LoRaMacFlags.Bits.MacDone = 0;
						
				/************************* add buy Jason ************************/                              
				if(Sx127xHandle.SendAgain) ///接收数据失败，重发机制
				{
					// Sends the same frame again
					DEBUG_APP(2,"ScheduleTx Start");
					TimerStop( &CsmaTimer );
					TimerSetValue( &CsmaTimer, 10*TIMEONAIR + randr(-10, 10)*TIMEONAIR); ///ALOHA
					TimerStart( &CsmaTimer );

					DEBUG_APP(2,"TimeOnAir = %d", 10*TIMEONAIR + randr(-10, 10)*TIMEONAIR);                  
				...
				else ///接收到数据
				{
					...
					DEBUG_APP(2,"LoRaMacState &= ~MAC_TX_RUNNING--7 %d",LoRaMacState);
				}
			/************************* add buy Jason ************************/   
			}
			else
    ...
    #error "Please define a frequency band in the compiler options."
				...
				DEBUG_APP(2,"LoRaMacState &= ~MAC_TX_RUNNING--5 %d",LoRaMacState);
       }
    }
	}
...
void OnRxWindow1TimerEvent( void )
{
	...
	RxWindowSetup( (Channels[User.Sx127xRx1Channel].Frequency), datarate, bandwidth, symbTimeout, false );//false---true连续接收
	DEBUG_APP(2,"RxWindow1 Frequency %d, datarate %d",(Channels[User.Sx127xRx1Channel].Frequency),datarate);
...
//  RxWindowSetup( Rx2Channel.Frequency, Rx2Channel.Datarate, bandwidth, symbTimeout, false ); 
		RxWindowSetup( (Channels[User.Sx127xRx1Channel].Frequency), datarate, bandwidth, symbTimeout, false); ///强制切换
	}
	else
	{
//    RxWindowSetup( Rx2Channel.Frequency, Rx2Channel.Datarate, bandwidth, symbTimeout, true );
					
		RxWindowSetup( (Channels[User.Sx127xRx2Channel].Frequency), datarate, bandwidth, symbTimeout, true );
		DEBUG_APP(2,"RX2Frequency = %d, Datarate = %d",Channels[User.Sx127xRx2Channel].Frequency,datarate);
	}
	...
	if( AggregatedTimeOff < ( curTime - AggregatedLastTxDoneTime ) )
	{
		...
		for( uint8_t i = 0; i < LORA_MAX_NB_BANDS; i++ )
		{
			if( DutyCycleOn == true )
			{
				...
				{
					nextTxDelay = MIN( Bands[i].TimeOff - ( curTime - Bands[i].LastTxDoneTime ),
														 nextTxDelay );
				}
			}
			else
...
static void RxWindowSetup( uint32_t freq, int8_t datarate, uint32_t bandwidth, uint16_t timeout, bool rxContinuous )
{
	...
	if( Radio.GetStatus( ) == RF_IDLE )
	{
		...
		DEBUG_APP(2,"RxWindowSetup freq = %d",freq);
		...
		else
		{
			...
			if(LoRaCsma.Iq_Invert) ///点对点
			{
				 Radio.SetRxConfig( modem, bandwidth, downlinkDatarate, 1, 0, 8, timeout, false, 0, false, 0, 0, false, rxContinuous ); //false -- P2P
				 DEBUG_APP(3,"Radio.SetRxConfig");
			}
			else
			{
...
//            Radio.SetRxConfig( modem, bandwidth, downlinkDatarate, 1, 0, 8, timeout, false, 0, false, 0, 0, true, rxContinuous );
	...
	{
		DEBUG_APP(2,"Radio.MaxRxWindow: %d",MaxRxWindow);
		Radio.Rx( MaxRxWindow ); 
	}
	else
	{
		DEBUG_APP(3,"Radio.Rx");
...
static bool ValidatePayloadLength( uint8_t lenN, int8_t datarate, uint8_t fOptsLen )
{
	...
	if( RepeaterSupport == true )
	{
		...
		Sx127xHandle.UpdateAdr = datarate;
	}
	else
	{
		maxN = MaxPayloadOfDatarate[datarate];
		Sx127xHandle.UpdateAdr = datarate; ///datarate回调使用
	}
...
LoRaMacStatus_t ScheduleTx( )
{
    ...
    {
        // Try to send now
        DEBUG_APP(3,"SendFrameOnChannel");
//        return SendFrameOnChannel( Channels[Channel] );
			return SendFrameOnChannel( Channels[User.Sx127xTxChannel] );
    }
    else
    {
...
LoRaMacStatus_t PrepareFrame( LoRaMacHeader_t *macHdr, LoRaMacFrameCtrl_t *fCtrl, uint8_t fPort, void *fBuffer, uint16_t fBufferSize )
{
    ...
    switch( macHdr->Bits.MType )
    {
        ...
        case FRAME_TYPE_DATA_UNCONFIRMED_UP:
            ...
            DEBUG_APP(3,"SrvAckRequested : %d",SrvAckRequested);
            if( SrvAckRequested == true )
            {
                SrvAckRequested = false;
                fCtrl->Bits.Ack = 1;
                DEBUG_APP(3,"fCtrl->Bits.Ack : %d",fCtrl->Bits.Ack);
            }
            ...
            if( ( payload != NULL ) && ( payloadSize > 0 ) )
            {
                ...
                {
                    DEBUG_APP(2,"fCtrl->Bits.FOptsLen : %d Index :%d",fCtrl->Bits.FOptsLen,MacCommandsBufferIndex);
            ...
            if( ( payload != NULL ) && ( payloadSize > 0 ) )
            {
                ...
                if( framePort == 0 )
                {
                    LoRaMacPayloadEncrypt( (uint8_t* ) payload, payloadSize, LoRaMacNwkSKey, LoRaMacDevAddr, UP_LINK, (uint16_t)UpLinkCounter, LoRaMacPayload );
                }
                else
                {
                    LoRaMacPayloadEncrypt( (uint8_t* ) payload, payloadSize, LoRaMacAppSKey, LoRaMacDevAddr, UP_LINK, (uint16_t)UpLinkCounter, LoRaMacPayload );
                }
                memcpy1( LoRaMacBuffer + pktHeaderLen, LoRaMacPayload, payloadSize );
            }
            LoRaMacBufferPktLen = pktHeaderLen + payloadSize;

            LoRaMacComputeMic( LoRaMacBuffer, LoRaMacBufferPktLen, LoRaMacNwkSKey, LoRaMacDevAddr, UP_LINK, (uint16_t)UpLinkCounter, &mic );
...
LoRaMacStatus_t SendFrameOnChannel( ChannelParams_t channel )
{
    ...
    McpsConfirm.TxPower = ChannelsTxPower;
		...
		DEBUG_APP(2,"McpsConfirm.Datarate = %d datarate = %d",McpsConfirm.Datarate, datarate);    

		Radio.SetChannel( channel.Frequency );
		DEBUG_APP(2, "Txchannel.Frequency = %d",channel.Frequency);
    ...
    MlmeConfirm.TxTimeOnAir = TxTimeOnAir;
		...
		DEBUG_APP(2,"McpsConfirm.TxTimeOnAir = %d, MlmeConfirm.TxTimeOnAir = %d",McpsConfirm.TxTimeOnAir,MlmeConfirm.TxTimeOnAir);
...
LoRaMacStatus_t LoRaMacInitialization( LoRaMacPrimitives_t *primitives, LoRaMacCallback_t *callbacks )
{
    ...
    ChannelsDefaultDatarate = ChannelsDatarate = Sx127xHandle.DefaultDatarate; //LORAMAC_DEFAULT_DATARATE;
		
		DEBUG_APP(2,"Sx127xHandle DefaultDatarate = %d",Sx127xHandle.DefaultDatarate);
    ChannelsNbRep = 1;
    ChannelsNbRepCounter = 0;
...
LoRaMacStatus_t LoRaMacMibGetRequestConfirm( MibRequestConfirm_t *mibGet )
{
    ...
    switch( mibGet->Type )
    {
        ...
        case MIB_MULTICAST_CHANNEL:
        {
            ...
            DEBUG_APP(2,"MIB_MULTICAST_CHANNEL ");
            break;
        }
        default:
...
LoRaMacStatus_t LoRaMacMulticastChannelLink( MulticastParams_t *channelParam )
{
    ...
    {
    	DEBUG_APP(2,"MulticastChannels NULL ");
        return LORAMAC_STATUS_PARAMETER_INVALID;
    }
    if( ( LoRaMacState & MAC_TX_RUNNING ) == MAC_TX_RUNNING )
    {
    	DEBUG_APP(2,"LORAMAC_STATUS_BUSY NULL ");
        return LORAMAC_STATUS_BUSY;
    }
...
LoRaMacStatus_t LoRaMacMcpsRequest( McpsReq_t *mcpsRequest )
{
    ...
    if( mcpsRequest == NULL )
    {
        DEBUG_APP(3,"LORAMAC_STATUS_PARAMETER_INVALID ");
        return LORAMAC_STATUS_PARAMETER_INVALID;
    }
    if( ( ( LoRaMacState & MAC_TX_RUNNING ) == MAC_TX_RUNNING ) ||
        ( ( LoRaMacState & MAC_TX_DELAYED ) == MAC_TX_DELAYED ) )
    {
        DEBUG_APP(2,"LoRaMacState ： %d ",LoRaMacState);
        return LORAMAC_STATUS_BUSY;
    }
