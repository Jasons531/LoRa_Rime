...
{ 
	...
	GPIO_InitStructure.Pin = 0xf103;  ///0xF0FF：串口有打印  0xFDFF 0xFF9F  0xFB0F--- UART1-PA10(RX) SPI 不设置为模拟输入   0xFF0F GPIO_PIN_All
...
{ 
	...
	GPIO_InitStructure.Pin 		= 0xFDFE;  /// PB9：CH_CE(ok)/PB5：485_DE/PB0: RESET/PB1: DIO0 保留充电IO配置
	GPIO_InitStructure.Mode 	= GPIO_MODE_ANALOG;
	GPIO_InitStructure.Pull 	= GPIO_PULLDOWN;
	GPIO_InitStructure.Speed  = GPIO_SPEED_FREQ_LOW;	
	HAL_GPIO_Init(GPIOB, &GPIO_InitStructure);
