...
void SI7021ReadTH(int32_t *measure)
{
	...
	SI7021_Configtrue(SI7021_Configuration);  ///配置SI7021_Configuration，初始化
	DelayMs( 15 );
	
	SI7021_WriteOneByte(SERIAL2_READ, 2); ///设置CRC字节
	
	FID = SI7021_WriteOneByte(Firmware_ID, 2); ///读取版本ID
	DelayMs( 15 );
...
/*
 *	SI7021_CheckCrc:	对sht2x读出的数据进行校验
 *	measure：		成功：1；失败：0
 */
