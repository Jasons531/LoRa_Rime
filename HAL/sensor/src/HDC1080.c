...
void Hdc1080ReadTH(int32_t *measure)
{
	...
	uint16_t MID = Hdc1080_ReadOneByte(HDC_Manufacturer_ID);
	DelayMs( 1 );
	
	uint16_t DID = Hdc1080_ReadOneByte(HDC_Device_ID);
	...
	if(MID == Manufacturer_ID_value &&  DID == Device_ID_value) ///初始化完成
	{
		...
		for(i=0;i<10;i++)
		{
			Temperature_Data = Hdc1080_ReadOneByte(Temperature);
		...
		for(i=0;i<10;i++)
		{		
			Humidity_Data = Hdc1080_ReadOneByte(Humidity);
...
/*Hdc1080_ReadOneByte ：  读入一个字节
 *reg:										寄存器地址
 *返回值:                 0：失败  非0：成功
*/  
uint16_t Hdc1080_ReadOneByte(uint8_t reg)
{
	uint8_t data[2];
	uint16_t Had_Data = 0;
	HAL_StatusTypeDef state;
